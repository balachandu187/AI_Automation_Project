...
export default function Layout() {
  ...
  const isEditor = location.pathname.startsWith("/workflows/");

  return (
    <div className="min-h-screen flex flex-col">
      {/* Top nav — hide on editor page to save space */}
      {!isEditor && (
        <header className="border-b border-surface-700 bg-surface-900">
          <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
            <Link
              to="/"
              className="flex items-center gap-2 text-lg font-bold text-flowmind-400"
            >
              <svg
                width="28"
                height="28"
      ...
      {/* Main content */}
      <main className={`flex-1 ${isEditor ? "h-[100vh]" : ""}`}>
        <Outlet />
      </main>
    </div>
  );
